...
import { EMAIL, GITHUB_URL, LINKEDIN_URL, GITHUB_HANDLE } from '../data/config'

const socialLinks = [
  {
    name: 'GitHub', href: GITHUB_URL, tooltip: `github.com/${GITHUB_HANDLE}`,
  ...
  {
    name: 'LinkedIn', href: LINKEDIN_URL, tooltip: LINKEDIN_URL.replace('https://', ''),
    icon: <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
  },
  {
    name: 'Email', href: `mailto:${EMAIL}`, tooltip: EMAIL,
    icon: <><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" fill="none" stroke="currentColor" strokeWidth="2" /><polyline points="22,6 12,13 2,6" fill="none" stroke="currentColor" strokeWidth="2" /></>
  },
]
...
export function Contact() {
  ...
  const [submitStatus, setSubmitStatus] = useState(null) // 'success' | error message string | null
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitStatus(err.message || 'SIGNAL LOST · Failed to send. Please try again.')
    } finally {
      ...
      <motion.div style={{ maxWidth: '600px', margin: '0 auto' }} variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }}>
        ...
        <motion.form style={{ textAlign: 'left', backgroundColor: 'var(--ui-panel-bg)', border: '1px solid var(--ui-input-border)', padding: '2rem' }} onSubmit={handleSubmit} variants={fadeUpVariants}>
          ...
          {submitStatus === 'success' && (
            <div style={{ marginTop: '1rem', padding: '0.75rem 1rem', border: '1px solid rgba(74, 222, 128, 0.3)', backgroundColor: 'rgba(74, 222, 128, 0.07)', fontFamily: "'Share Tech Mono', monospace", fontSize: '0.85rem', color: '#4ade80', letterSpacing: '0.05em' }}>
              TRANSMISSION SUCCESSFUL · Message received. I'll be in touch shortly.
            </div>
          )}
          {submitStatus && submitStatus !== 'success' && (
            <div style={{ marginTop: '1rem', padding: '0.75rem 1rem', border: '1px solid rgba(224, 80, 80, 0.3)', backgroundColor: 'rgba(224, 80, 80, 0.07)', fontFamily: "'Share Tech Mono', monospace", fontSize: '0.85rem', color: '#e05050', letterSpacing: '0.05em' }}>
              {submitStatus}
            </div>
          )}
        </motion.form>
